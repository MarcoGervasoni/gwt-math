...
/**
 * Immutable, arbitrary-precision signed decimal numbers. A BigDecimal consists
 * of an arbitrary precision integer <i>unscaled value</i> and a non-negative
 * 32-bit integer <i>scale</i>, which represents the number of digits to the
 * right of the decimal point. The number represented by the BigDecimal is
 * <tt>(unscaledValue/10<sup>scale</sup>)</tt>. BigDecimal provides
 * operations for basic arithmetic, scale manipulation, comparison, hashing, and
 * format conversion.
 * <p>
 * The BigDecimal class gives its user complete control over rounding behavior,
 * forcing the user to explicitly specify a rounding behavior for operations
 * capable of discarding precision ({@link #divide(BigDecimal, int)},
 * {@link #divide(BigDecimal, int, int)}, and {@link #setScale}). Eight
 ...
 * Scaling/rounding operations (<tt>setScale</tt>) return a BigDecimal whose
 * value is approximately (or exactly) equal to that of the operand, but whose
 * scale is the specified value; that is, they increase or decrease the
 * precision of the number with minimal effect on its value. Decimal point
 * motion operations ({@link #movePointLeft} and {@link #movePointRight})
 * return a BigDecimal created from the operand by moving the decimal point a
 * specified distance in the specified direction; that is, they change a
 * number's value without affecting its precision.
 * <p>
 * For the sake of brevity and clarity, pseudo-code is used throughout the
 * descriptions of BigDecimal methods. The pseudo-code expression
 * <tt>(i + j)</tt> is shorthand for "a BigDecimal whose value is that of the
 * BigDecimal <tt>i</tt> plus that of the BigDecimal <tt>j</tt>." The
 * pseudo-code expression <tt>(i == j)</tt> is shorthand for "<tt>true</tt>
 * if and only if the BigDecimal <tt>i</tt> represents the same value as the
 * the BigDecimal <tt>j</tt>." Other pseudo-code expressions are interpreted
 * similarly.
 * <p>
 * Note: care should be exercised if BigDecimals are to be used as keys in a
 * {@link java.util.SortedMap} or elements in a {@link java.util.SortedSet}, as
 * BigDecimal's <i>natural ordering</i> is <i>inconsistent with equals</i>.
 * See {@link Comparable}, {@link java.util.SortedMap} or
 * {@link java.util.SortedSet} for more information.
 * <p>
 * All methods and constructors for this class throw
 * <CODE>NullPointerException</CODE> when passed a null object reference for
 * any input parameter.
 * 
 * @see BigInteger
...
public class BigDecimal extends Number implements Comparable {
	 ...
	 * The String representation consists of an optional sign, <tt>'+'</tt> (<tt>'&#92;u002B'</tt>)
	 * or <tt>'-'</tt> (<tt>'&#92;u002D'</tt>), followed by a sequence of
	 ...
	 * 
	 * <p>
	 * The exponent consists of the character <tt>'e'</tt> (<tt>'&#92;u0075'</tt>)
	 * or <tt>'E'</tt> (<tt>'&#92;u0045'</tt>) followed by one or more
	 * decimal digits. The value of the exponent must lie between -{@link Integer#MAX_VALUE} ({@link
	 ...
	 * <code>true</code>, including 0, 1, 2 ...
	 * </dl>
	 * </blockquote>
	 * 
	 * <p>
	 ...
	 * adjustment for any exponent: If the string contains an exponent, the
	 ...
	 * case, the resulting BigDecimal is equal to <i>significand</i> &times; 10<i><sup>exponent</sup></i>.
	 * (If in the future this specification is amended to permit negative
	 * scales, the final step of zeroing the scale and adjusting the unscaled
	 ...
	 * 
	 * <p>
	 * Note: For values other <tt>float</tt> and <tt>double</tt> NaN and
	 ...
	 * the preferred way to convert a <tt>float</tt> or <tt>double</tt> into
	 * a BigDecimal, as it doesn't suffer from the unpredictability of the
	 * {@link #BigDecimal(double)} constructor.
	 * 
	 * <p>
	 * Note: the optional leading plus sign and trailing exponent were added in
	 * release 1.3.
	 * 
	 * @param val
	 *            String representation of BigDecimal.
	 * @throws NumberFormatException
	 *             <tt>val</tt> is not a valid representation of a BigDecimal.
	 */
	public BigDecimal(String val) {
		...
		if (pointPos == -1) { /* e.g. "123" */
			...
			//val.getChars(0, pointPos, digits, 0); SILVIO
			getChars(val, 0, pointPos, digits, 0);
			// Get chars after decimal point
			//val.getChars(pointPos + 1, val.length(), digits, pointPos); SILVIO
			getChars(val, pointPos + 1, val.length(), digits, pointPos);
	...
	private void getChars(String value, int srcBegin, int srcEnd, char dst[], int dstBegin) {
        if (srcBegin < 0) {
        ...
        for(int i=srcBegin;i<srcEnd;i++) {
        	dst[dstBegin+y]=value.charAt(i);
        	y++;
        }
	}
	
	/**
	 * Translates a <code>double</code> into a BigDecimal. The scale of the
	 ...
	 * might assume that <tt>new BigDecimal(.1)</tt> is exactly equal to .1,
	 ...
	 * 
	 * @param val
	 *            <code>double</code> value to be converted to BigDecimal.
	 * @throws NumberFormatException
	 *             <tt>val</tt> if <tt>val</tt> is infinite or NaN.
	 */
	public BigDecimal(double val) {
		 ...
		 */
		//long valBits = Double.doubleToLongBits(val); SILVIO
		long valBits = Long.parseLong(val+"");
	...
	/**
	 * Translates a BigInteger into a BigDecimal. The scale of the BigDecimal is
	 * zero.
	 * 
	 * @param val
	...
	/**
	 * Translates a BigInteger unscaled value and an <code>int</code> scale
	 * into a BigDecimal. The value of the BigDecimal is
	...
	/**
	 * Translates a <code>long</code> unscaled value and an <code>int</code>
	 * scale into a BigDecimal. This &quot;static factory method&quot; is
	 * provided in preference to a (<code>long</code>, <code>int</code>)
	...
	/**
	 * Translates a <code>long</code> value into a BigDecimal with a scale of
	 * zero. This &quot;static factory method&quot; is provided in preference to
	 * a (<code>long</code>) constructor because it allows for reuse of
	...
	/**
	 * Returns a BigDecimal whose value is <tt>(this / val)</tt>, and whose
	 * scale is as specified. If rounding must be performed to generate a result
	 * with the specified scale, the specified rounding mode is applied.
	 * 
	 * @param val
	...
	public BigDecimal divide(BigDecimal val, int scale, int roundingMode) {
		...
		return (increment ? new BigDecimal(q.add(BigInteger.valueOf(signum)),
				scale) : new BigDecimal(q, scale));
	}
	...
	/**
	 * Returns the <i>scale</i> of this BigDecimal. (The scale is the number of
	 ...
	 * usual convention of having methods named <code>set<i>X</i></code>
	 * mutate field <code><i>X</i></code>. Instead, <code>setScale</code>
	...
	/**
	 * Returns a BigDecimal whose scale is the specified value, and whose value
	 * is numerically equal to this BigDecimal's. Throws an ArithmeticException
	 * if this is not possible. This call is typically used to increase the
	 * scale, in which case it is guaranteed that there exists a BigDecimal of
	 * the specified scale and the correct value. The call can also be used to
	 * reduce the scale if the caller knows that the BigDecimal has sufficiently
	 * many zeros at the end of its fractional part (i.e., factors of ten in its
	 * integer value) to allow for the rescaling without loss of precision.
	 * <p>
	 * This method returns the same result as the two argument version of
	 * setScale, but saves the caller the trouble of specifying a rounding mode
	 * in cases where it is irrelevant.
	 * <p>
	 * Note that since BigDecimal objects are immutable, calls of this method do
	 * <i>not</i> result in the original object being modified, contrary to the
	 * usual convention of having methods named <code>set<i>X</i></code>
	 ...
	 * this BigDecimal's scale is &gt;= <tt>n</tt>, the call merely subtracts
	 * <tt>n</tt> from the scale; otherwise, it sets the scale to zero, and
	 * multiplies the integer value by <tt>10<sup>(n - this.scale)</sup></tt>.
	 * If <tt>n</tt> is negative, the call is equivalent to
	 * <tt>movePointLeft(-n)</tt>. (The BigDecimal returned by this call has
	...
	/**
	 * Compares this BigDecimal with the specified BigDecimal. Two BigDecimals
	 * that are equal in value but have a different scale (like 2.0 and 2.00)
	 * are considered equal by this method. This method is provided in
	 * preference to individual methods for each of the six boolean comparison
	 * operators (&lt;, ==, &gt;, &gt;=, !=, &lt;=). The suggested idiom for
	 * performing these comparisons is: <tt>(x.compareTo(y)</tt> &lt;<i>op</i>&gt;
	 * <tt>0)</tt>, where &lt;<i>op</i>&gt; is one of the six comparison
	 * operators.
	 * 
	 * @param val
	 ...
	 * @return a negative number, zero, or a positive number as this BigDecimal
	 *         is numerically less than, equal to, or greater than <tt>o</tt>,
	 *         which must be a BigDecimal.
	 * @throws ClassCastException
	 *             <tt>o</tt> is not a BigDecimal.
	 ...
	 * @return the BigDecimal whose value is the greater of this BigDecimal and
	 *         <tt>val</tt>. If they are equal, as defined by the
	 *         {@link #compareTo compareTo} method, either may be returned.
	 * @see #compareTo(java.math.BigDecimal)
	 */
	...
	/**
	 * Returns the hash code for this BigDecimal. Note that two BigDecimals that
	 * are numerically equal but differ in scale (like 2.0 and 2.00) will
	 ...
	 * the right of the decimal point is used to indicate scale. (This
	...
	/**
	 * Converts this BigDecimal to a BigInteger. This conversion is analogous to
	 * a <a
	 * href="http://java.sun.com/docs/books/jls/second_edition/html/conversions.doc.html#25363"><i>narrowing
	 * primitive conversion</i></a> from <code>double</code> to
	 * <code>long</code> as defined in the <a
	 * href="http://java.sun.com/docs/books/jls/html/">Java Language
	 * Specification</a>: any fractional part of this BigDecimal will be
	 * discarded. Note that this conversion can lose information about the
	 * precision of the BigDecimal value.
	 * 
	 * @return this BigDecimal converted to a BigInteger.
	 */
	public BigInteger toBigInteger() {
		return (scale == 0 ? intVal : intVal.divide(BigInteger.valueOf(10).pow(
				scale)));
	}

	/**
	 * Converts this BigDecimal to an <code>int</code>. This conversion is
	 * analogous to a <a
	 * href="http://java.sun.com/docs/books/jls/second_edition/html/conversions.doc.html#25363"><i>narrowing
	 * primitive conversion</i></a> from <code>double</code> to
	 ...
	 * href="http://java.sun.com/docs/books/jls/html/">Java Language
	 * Specification</a>: any fractional part of this BigDecimal will be
	 * discarded, and if the resulting &quot;BigInteger&quot; is too big to fit
	 * in an <code>int</code>, only the low-order 32 bits are returned. Note
	 * that this conversion can lose information about the overall magnitude and
	 * precision of the BigDecimal value as well as return a result with the
	 * opposite sign.
	 * 
	...
	/**
	 * Converts this BigDecimal to a <code>long</code>. This conversion is
	 * analogous to a <a
	 * href="http://java.sun.com/docs/books/jls/second_edition/html/conversions.doc.html#25363"><i>narrowing
	 * primitive conversion</i></a> from <code>double</code> to
	 * <code>short</code> as defined in the <a
	 ...
	 * Specification</a>: any fractional part of this BigDecimal will be
	 * discarded, and if the resulting &quot;BigInteger&quot; is too big to fit
	 * in a <code>long</code>, only the low-order 64 bits are returned. Note
	 * that this conversion can lose information about the overall magnitude and
	 * precision of the BigDecimal value as well as return a result with the
	 * opposite sign.
	 * 
	...
	/**
	 * Converts this BigDecimal to a <code>float</code>. This conversion is
	 * similar to the <a
	 * href="http://java.sun.com/docs/books/jls/second_edition/html/conversions.doc.html#25363"><i>narrowing
	 * primitive conversion</i></a> from <code>double</code> to
	 * <code>float</code> defined in the <a
	 * href="http://java.sun.com/docs/books/jls/html/">Java Language
	 * Specification</a>: if this BigDecimal has too great a magnitude to
	 ...
	 * conversion can lose information about the precision of the BigDecimal
	 * value.
	 * 
	...
	/**
	 * Converts this BigDecimal to a <code>double</code>. This conversion is
	 * similar to the <a
	 * href="http://java.sun.com/docs/books/jls/second_edition/html/conversions.doc.html#25363"><i>narrowing
	 * primitive conversion</i></a> from <code>double</code> to
