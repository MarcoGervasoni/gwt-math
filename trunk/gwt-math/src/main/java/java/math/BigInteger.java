 ...
 * provided by this class ensures that there are infinitely many "virtual sign
 * bits" preceding each BigInteger.
 * <p>
 ...
 * if and only if the BigInteger <tt>i</tt> represents the same value as the
 * the BigInteger <tt>j</tt>." Other pseudo-code expressions are interpreted
 * similarly.
 * <p>
...
public class BigInteger extends Number implements Comparable {
	 ...
	 *             in the specified radix, or <tt>radix</tt> is outside the
	 *             range from {@link Character#MIN_RADIX} to
	...
	/**
	 * Returns a BigInteger whose value is <tt>(this | val)</tt>. (This
	 * method returns a negative BigInteger if and only if either this or val is
	 * negative.)
	 * 
	 * @param val
	...
	/**
	 * Returns a BigInteger whose value is <tt>(this ^ val)</tt>. (This
	 * method returns a negative BigInteger if and only if exactly one of this
	 ...
	 * BigInteger if and only if <tt>this</tt> is negative and <tt>val</tt>
	 * is positive.)
	 * 
	 * @param val
	...
	public String toString(int radix) {
		...
		while (tmp.signum != 0) {
			...
			//digitGroup[numGroups++] = Long.toString(r2.longValue(), radix); SILVIO
			digitGroup[numGroups++] = LongToString(r2.longValue(), radix);
			tmp = q2;
		}
	...
	private String LongToString(long i, int radix) {
		if (radix < Character.MIN_RADIX || radix > Character.MAX_RADIX)
		    radix = 10;
	        if (radix == 10)
